...
let ticketPriceTotal = runners.reduce((donationTotal, runnerDonation) => {
    return donationTotal + runnerDonation.donation
},0);
...
// Problem 1
//There is as need for a least of all the Company names of the runners involved
const CompanyList = [];

runners.forEach(runner => CompanyList.push(`${runner.first_name} ${runner.last_name}, their company is ${runner.company_name}`))
console.log(CompanyList)

// Problem 2
//Now the company names have to be in ALL CAPS
allCapsCompany = [];

runners.map(company => {
    return allCapsCompany.push(`${company.company_name.toUpperCase()}`)
})
console.log(allCapsCompany)

// Problem 3
//A list of runners with shirt size of XS
let xtraSmasll = runners.filter(runner => {
    return runner.shirt_size === "XS"
})
console.log(xtraSmasll);

//Problem 4
//Total of donations for runners with shirt size small
let donationTotalForXS = xtraSmasll.reduce((donationTotal, runnerDonation) => {
    return donationTotal + runnerDonation.donation
},0);

console.log(donationTotalForXS)